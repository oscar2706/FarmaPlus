...
public class CustomToast {
    ...
    public static void showOkToastRepartidor(Activity activity, String mensaje){
        LayoutInflater inflater = activity.getLayoutInflater();
        View toast_layout = inflater.inflate(R.layout.toast_pedido_entregado,
                (ViewGroup)activity.findViewById(R.id.toast_ok_repartidor));
        final Toast toast = new Toast(activity.getApplicationContext());
        TextView textView = toast_layout.findViewById(R.id.textView_toastOkRepartidor);
        textView.setText(mensaje);
        toast.setGravity(Gravity.CENTER_VERTICAL, 0, 0);
        toast.setDuration(Toast.LENGTH_LONG);
        toast.setView(toast_layout);
